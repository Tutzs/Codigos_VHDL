...
begin
    ...
    handle_s: seg7_dec port map(A=>s, seg=>seg7_s);
    handle_a: seg7_dec port map(A=>a, seg=>seg7_a);
    handle_b: seg7_dec port map(A=>b, seg=>seg7_b);
    
    handle_two_complement: comp2 port map(a=>b, sel=>sel, s=>current_b);
    
    sum0: som1 port map(a=>a(0), b=>current_b(0), cin=>'0', s=>s(0), cout=>c0);
    sum1: som1 port map(a=>a(1), b=>current_b(1), cin=>c0, s=>s(1), cout=>c1);
    sum2: som1 port map(a=>a(2), b=>current_b(2), cin=>c1, s=>s(2), cout=>c2);
    sum3: som1 port map(a=>a(3), b=>current_b(3), cin=>c2, s=>s3, cout=>c3);
    
    s(3) <= s3;
    has_overflow: ovf_detec port map(a=>a(3), b=>current_b(3), c=>s3, e=>e);
